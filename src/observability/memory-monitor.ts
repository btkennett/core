...
export class MemoryMonitor {
  ...
  private readonly warningCooldown = 30_000;
  ...
  getMemoryUsage(): {
    ...
    const formatted = Object.fromEntries(
      Object.entries(stats).map(([k, v]) => [k, `${v}MB`])
    ) as Record<keyof MemoryStats, string>;

    const warnings: string[] = [];

    if (stats.rss >= this.config.criticalThreshold) {
      warnings.push(
        `CRITICAL: RSS ${stats.rss}MB >= ${this.config.criticalThreshold}MB`
      );
    } else if (stats.rss >= this.config.warningThreshold) {
      warnings.push(
        `WARNING: RSS ${stats.rss}MB >= ${this.config.warningThreshold}MB`
      );
    }

    if (stats.heapUsed >= this.config.criticalThreshold * 0.8) {
      warnings.push(
        `CRITICAL: Heap ${stats.heapUsed}MB >= 80% of critical threshold`
      );
    } else if (stats.heapUsed >= this.config.warningThreshold * 0.8) {
      warnings.push(
        `WARNING: Heap ${stats.heapUsed}MB >= 80% of warning threshold`
      );
    }
  ...
  logMemoryUsage(): void {
    ...
    if (warnings.length > 0) {
      ...
      if (now - this.lastWarningTime >= this.warningCooldown) {
        this.logger.warn(
          `[Memory] RSS: ${formatted.rss}, Heap: ${formatted.heapUsed}/${formatted.heapTotal}`
        );
      ...
      this.logger.log(
        `[Memory] RSS: ${formatted.rss}, Heap: ${formatted.heapUsed}/${formatted.heapTotal}, External: ${formatted.external}`
      );
    }
  }
  ...
  getHealthStatus(): MemoryHealth {
    ...
    return {
      healthy:
        stats.rss < this.config.criticalThreshold &&
