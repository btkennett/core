 ...
 */
function extractColumnHeaders(tableLines) {
 ...
 */
function findNearestHeader(text, position) {
...
export function chunkText(text, options = {}) {
    const { maxTokens = 800, minTokens = 300, overlap = 100, splitOn = ["\n\n", "\n", ". ", " "], } = options;
    ...
    while (position < cleanText.length) {
        ...
        position = Math.max(endPosition - overlapChars, position + 1 // Ensure we always make progress
        );
    }
    return chunks;
}
 ...
 */
export function tableAwareChunk(text, options = {}) {
        ...
        for (const match of matches) {
            ...
            if (table) {
                ...
                if (table.columnHeaders)
                    allHeaders.push(table.columnHeaders);
            }
        }
 ...
 */
export function chunkTextWithMetadata(text, metadata, options = {}) {
 ...
 */
export function chunkBySections(text, sectionMarkers = /^#+\s+.+$/gm, // Markdown headers by default
options = {}) {
    const sections = [];
